...
**FormSubmit configuration record (2026-08-20):** Reworked the existing FormSubmit AJAX adapter without changing the enquiry recipient. Added `VITE_FORM_SUBMIT_ID` as the clear public configuration point for the random replacement string from the activation email; it takes effect in both the rendered form action and AJAX endpoint, while the direct recipient remains only as a temporary fallback so the Pages deployment is not broken during transition. Standardized `_subject` as `New Website Enquiry - Surya Amor Technology`, retained the `table` template and Reply-To, added clearly labelled Name, Company, Email, Phone, Service Interested In, Message, privacy acknowledgement, Malaysia timestamp and source values, and checks JSON-level provider failures before clearing the form. Replaced the ineffective always-empty honeypot with a real visually hidden `_honey` field and restored FormSubmit's default CAPTCHA protection in line with its official recommendation. The existing accessible validation and exact in-page success message remain; no `_next` is needed for the AJAX flow. Responsive behavior remains within the shared two-column desktop and one-column mobile form grid. A production build with a temporary non-secret identifier confirmed endpoint substitution, after which the standard production build was restored. The activation email was received for `surya-amor-website.pages.dev`, but the identifier itself is not in the project and mailbox delivery was not tested because sending a real enquiry is an external action. Manual deployment steps: add `VITE_FORM_SUBMIT_ID` to Cloudflare Pages Preview and Production, set production `VITE_SITE_URL` to `https://www.satsb.com.my`, redeploy, submit non-sensitive tests from both origins, and approve another activation request if FormSubmit issues one for the new domain.

**Brand, media, navigation and copy-balance record (2026-08-20):** Inspected the supplied transparent 447 × 123 `SAT LOGO -BLUE.png`, integrated it through the reusable `CompanyLogo` component for desktop, mobile and footer display, updated intrinsic dimensions, and confirmed no code or documentation references remained before removing the previous pink logo file. Rebuilt the shared primary navigation as Home, About, Services, Industries, Hardware, Clients, News, Careers and Contact; both header modes and the footer Explore list now consume the same `navigation` data, while service links and legal links remain separately grouped. Added a modest Clients introduction and bottom trust note without testimonials, ratings or quantified claims. Reduced repeated prose across Home, About, Services, service workflows, Industries, Hardware, Contact, reusable CTAs and related-service sections while preserving technical data and factual meaning. Constrained the News feature image to 66rem/34rem and gallery cards to a centered two-column 66rem/28rem layout, removed forced crops and full-width gallery emphasis, preserved intrinsic ratios, and added mobile max-height rules. Because no local audio/video encoder was available, no third-party track was downloaded or embedded. Instead, the video gallery now offers an optional original Web Audio ambient pad created entirely in project code; it starts only after the visitor enables it and plays a video, fades on pause/end/clip change, remains low beneath original clip audio, and requires no external music licence or attribution. Direct video controls remain unchanged. ESLint, strict TypeScript and the production build passed. The production preview returned HTTP 200 for all 12 primary, News, legal and contact routes tested, the new logo and all eight supplied News media files; video range delivery returned HTTP 206. Shared navigation targets were compared with the application route table. Browser-based listening, visual breakpoint and console checks remain pending because no browser executable is installed.
...
- [x] Add random FormSubmit identifier configuration, fixed subject/table fields and functional spam protection
- [x] Align desktop, mobile and footer page navigation through shared data
...
- [x] Replace the former website identity and increase all displayed logo widths by 20%
...
- [x] Replace the company identity with the supplied blue logo and remove the unused prior asset
- [x] Constrain News article imagery and add optional original user-controlled ambient video audio
- [x] Add concise Clients context and reduce repetitive copy across major pages
...
- [ ] Add `VITE_FORM_SUBMIT_ID` to Cloudflare Pages Preview and Production and redeploy
- [ ] Verify FormSubmit mailbox delivery from `surya-amor-website.pages.dev` and `www.satsb.com.my`; approve another domain activation request if prompted
